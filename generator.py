...
Ce module génère des images abstraites expressives à partir de fichiers CSV.
Il transforme des données numériques en compositions artistiques uniques avec
différents types de formes (blob, stroke, splatter, spiral, wave, cloud).
"""
...
class RowRNG:
    """
    Générateur de nombres pseudo-aléatoires déterministe basé sur SHA-256.

    Utilisé pour générer des valeurs aléatoires reproductibles à partir d'une seed.
    """
    ...
    def _next_byte(self):
        """Génère le prochain byte pseudo-aléatoire."""
    ...
    def uni(self, lo=0.0, hi=1.0):
        """Génère un nombre aléatoire uniforme entre lo et hi."""
        return lo + (hi - lo) * (self._next_byte() / 255.0)

    def choice(self, seq):
        """Sélectionne aléatoirement un élément de la séquence."""
...
def hsl_to_rgb(h, s, lightness):
    ...
    :param lightness: The parameter `lightness` in the `hsl_to_rgb` function represents the lightness value in the HSL
